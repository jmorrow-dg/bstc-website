#!/usr/bin/env python3
"""Generate the Airwallex x BSTC partnership deck as an editable PPTX.

Mirrors the HTML/PDF design: dark slides, Georgia display, BSTC red + Airwallex
orange. Run from repo root:  python3 public/generate-airwallex-deck.py
Output: public/airwallex-bstc-partnership-deck.pptx
"""
...
s.shapes.add_picture("public/images/airwallex-logo.png", Inches(LM), Inches(2.66), height=Inches(0.5))
block(s, 4.85, 2.55, 0.5, 0.6, [R("×", GREY2, font=SERIF, size=30)])
s.shapes.add_picture("public/images/bstc-badge.png", Inches(5.45), Inches(2.55), height=Inches(0.66))
block(s, 6.3, 2.55, 5, 0.7,
      [[R("BSTC", WHITE, bold=True, font=SERIF, size=27)],
       [R("BALI STARTUPS & TECH COMMUNITY", GREY, font=MONO, size=7.5)]], ls=1.0)
h1(s, LM, 3.55,
   [[R("Own the finance category in", WHITE, bold=True, font=SERIF)],
    [R("Bali's founder ecosystem.", RED, bold=True, font=SERIF)]], size=40)
meta = [("Prepared for", "Dushyant, Airwallex"), ("From", "BSTC"), ("Meeting", "Tuesday, 3:00pm")]
...
footer(s, "Airwallex × BSTC", "01")

# ============ 2 · WHY NOW ============
s = slide()
topbar(s, "Why now", "Airwallex × BSTC")
...
block(s, LM, 2.25, 11.4, 0.9,
      ["AI has democratised building. A single founder can now design, build and ship what used "
       ...
       "accelerating, and they are making their first financial decisions right now."],
...
block(s, LM, 5.55, 11.4, 0.7,
      ["They go global from day one, and they are choosing their financial stack now. "
       "The window to become their default is open."], size=13.5, color=SOFT, ls=1.4)
footer(s, "Why now", "02")

# ============ 3 · THE FIT ============
s = slide()
topbar(s, "Why this works", "Airwallex × BSTC")
kicker(s, LM, 1.55, "The thesis")
h1(s, LM, 1.95, [[R("You back founders building ", WHITE, bold=True, font=SERIF),
                 R("borderless", WHITE, bold=False, italic=True, font=SERIF),
                 R(" businesses.", WHITE, bold=True, font=SERIF)],
                [R("So do we.", RED, bold=True, font=SERIF)]])
block(s, LM, 3.85, 11.0, 1.4,
      ["Bali has quietly become a real hub for global-first startups. BSTC is its centre of "
       "gravity: 2,500+ founders, engineers and operators, most of them running cross-border, "
       "multi-currency businesses from day one. That is your customer, in one room, every month."],
      size=14, color=SOFT, ls=1.5)
footer(s, "The fit", "03")

# ============ 4 · COMMUNITY ============
s = slide()
topbar(s, "The community", "Airwallex × BSTC")
kicker(s, LM, 1.0, "Size & reach")
h1(s, LM, 1.34, [[R("Bali's founder community, ", WHITE, bold=True, font=SERIF),
...
for i, (n, lbl) in enumerate(stats):
    ...
    if "(confirm)" in lbl:
        block(s, x + 0.28, y + 0.95, bw - 0.5, 0.55,
              [[R("Events run in the last 12 months  ", GREY), R("(confirm)", ORANGE)]], size=10.5, ls=1.3)
    else:
...
# ============ 5 · GROWTH ============
s = slide()
topbar(s, "Momentum", "Airwallex × BSTC")
...
# ============ 6 · AUDIENCE ============
s = slide()
topbar(s, "The fit for Airwallex", "Airwallex × BSTC")
kicker(s, LM, 1.0, "Why Airwallex, specifically")
h1(s, LM, 1.34, [[R("Exactly who ", WHITE, bold=True, font=SERIF),
                 R("Airwallex", ORANGE, bold=True, font=SERIF),
                 R(" is built for", WHITE, bold=True, font=SERIF)]])
acols = [
    ("Who they are", "Global-first founders",
     "Founders and operators building from Bali but selling worldwide. Remote teams, international clients, multi-currency from launch."),
    ("What they need", "Borderless finance",
     "Cross-border payments, multi-currency accounts, global payroll and FX. The exact problems Airwallex solves, felt daily."),
    ("Where you fit", "The trusted answer",
     "Be the finance partner the community learns from and defaults to, through the scaling series you proposed and member perks."),
]
cw = (CW - 0.68) / 3
...
block(s, LM, 5.2, 11.2, 0.8,
      ["Your email named it: educational workshops on financial best practices for global "
       "operations. That is a perfect fit, and the start of something bigger."], size=13.5, color=SOFT, ls=1.45)
footer(s, "The audience", "06")

# ============ 7 · VISION ============
s = slide()
topbar(s, "Where we're going", "Airwallex × BSTC")
...
road = [
    ...
    ("2026", "The Podcast", "Launching soon. Founder stories and operator playbooks, with partner segments.", ORANGE),
    ("2026", "Owned insights", "Aggregated, anonymised ecosystem data from our member CRM.", ORANGE),
    ("2027", "Annual Conference", "Bali's flagship startup & tech conference. The headline stage.", ORANGE),
]
cw = CW / 4
...
# ============ 8 · PARTNERSHIP ============
s = slide()
topbar(s, "The proposal", "Airwallex × BSTC")
...
h1(s, LM, 1.34, [[R("Own the category. Be our ", WHITE, bold=True, font=SERIF),
                 ...
                 R(" finance partner.", WHITE, bold=True, font=SERIF)]])
block(s, LM, 2.3, 11.4, 0.7,
      [[R("Airwallex", ORANGE), R(" as the sole payments and finance brand across every BSTC surface "
        "for the term: events, workshops, podcast and the 2027 conference. No competing fintech in the room.", SOFT)]],
      size=13, ls=1.4)
incl = [
    ("Flagship event sponsorship", "Your brand on the monthly room that defines the scene."),
    ("The scaling series", "Co-built workshops on global financial best practice."),
    ("Founder perks", "Tailored Airwallex offers in our member guide and WhatsApp."),
    ("Podcast presence", "Partner segments and host reads as the show launches."),
    ("Conference 2027", "Headline finance partner of Bali's flagship event."),
    ("Ecosystem insights", "Aggregated, anonymised data on how Bali founders scale."),
]
iw = (CW - 0.44) / 3
ih = 1.25
...
s = slide()
topbar(s, "Ways to partner", "Airwallex × BSTC")
...
block(s, LM + 0.4, py + 1.16, qw - 0.8, 0.3, [R("Exclusive finance partner · one quarter", GREY, size=10)])
qitems = ["Category exclusivity for the quarter", "Flagship event sponsorship",
          "One scaling-series workshop", "Founder perks in the member guide"]
yy = py + 1.72
for it in qitems:
...
block(s, ax + 0.42, py + 1.16, aw - 0.84, 0.3, [R("Exclusive finance partner · the full year", GREY, size=10)])
aitems = ["Category exclusivity for the full year", "Every flagship night, all four formats",
          "Full scaling-series workshop track", "Podcast presence as it launches",
          "Headline finance partner, Conference 2027", "Aggregated ecosystem insights"]
yy = py + 1.72
for it in aitems:
...
# ============ 10 · CLOSE ============
s = slide()
topbar(s, "Next steps", "Airwallex × BSTC")
kicker(s, LM, 1.95, "Let's build it")
h1(s, LM, 2.35, [[R("Be the name Bali's founders", WHITE, bold=True, font=SERIF)],
                [R("reach for when they ", WHITE, bold=True, font=SERIF),
                 R("scale.", WHITE, bold=False, italic=True, font=SERIF)]], size=40)
block(s, LM, 4.35, 11.2, 0.9,
      ["You offered to support the community without a rigid sales pitch. We are offering you the "
       "whole category, for a year, including the stage we are building for 2027. Let's lock the "
       "shape of it on Tuesday."], size=13.5, color=SOFT, ls=1.45)
cta = [("The ask", "Annual category exclusivity"), ("The meeting", "Tuesday, 3:00pm"), ("Contact", "BSTC")]
...
footer(s, "Airwallex × BSTC · Partnership Proposal", "10")

prs.save("public/airwallex-bstc-partnership-deck.pptx")
print("Saved public/airwallex-bstc-partnership-deck.pptx —", len(prs.slides._sldIdLst), "slides")
